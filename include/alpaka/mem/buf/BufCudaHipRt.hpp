...
namespace alpaka
{
    ...
    namespace mem
    {
        namespace buf
        {
            ...
            public:
                ...
                ALPAKA_FN_HOST BufCudaHipRt(
                    ...
                    TExtent const & extent) :
                        ...
                        m_spMem(
                            pMem,
                            // NOTE: Because the BufCudaHipRt object can be copied and the original object could have been destroyed,
                            ...
                            std::bind(&BufCudaHipRt::freeBuffer, std::placeholders::_1, m_dev)),
            ...
            public:
                dev::DevCudaHipRt m_dev;               // NOTE: The device has to be destructed after the memory pointer because it is required for destruction.
    ...
    namespace dev
    {
        ...
        {
            //#############################################################################
            //! The BufCudaHipRt device type trait specialization.
            template<
                typename TElem,
                typename TDim,
                typename TIdx>
            struct DevType<
                mem::buf::BufCudaHipRt<TElem, TDim, TIdx>>
            {
                using type = dev::DevCudaHipRt;
            };
            //#############################################################################
            //! The BufCudaHipRt device get trait specialization.
            template<
                typename TElem,
                typename TDim,
                typename TIdx>
                ...
                mem::buf::BufCudaHipRt<TElem, TDim, TIdx>>
            {
                ALPAKA_FN_HOST static auto getDev(
                    mem::buf::BufCudaHipRt<TElem, TDim, TIdx> const & buf)
                -> dev::DevCudaHipRt
                {
                    return buf.m_dev;
                }
    ...
    namespace dim
    {
        ...
        {
            //#############################################################################
            //! The BufCudaHipRt dimension getter trait specialization.
            template<
                typename TElem,
                typename TDim,
                typename TIdx>
    ...
    namespace elem
    {
        ...
        {
            //#############################################################################
            //! The BufCudaHipRt memory element type get trait specialization.
            template<
                typename TElem,
                typename TDim,
                typename TIdx>
    ...
    namespace extent
    {
        ...
        {
            //#############################################################################
            //! The BufCudaHipRt extent get trait specialization.
            template<
                ...
                typename std::enable_if<(TDim::value > TIdxIntegralConst::value)>::type>
            {
                ...
                ALPAKA_FN_HOST static auto getExtent(
                    mem::buf::BufCudaHipRt<TElem, TDim, TIdx> const & extent)
                -> TIdx
                {
    ...
    namespace mem
    {
        namespace view
        {
            ...
            {
                //#############################################################################
                //! The BufCudaHipRt native pointer get trait specialization.
                template<
                    typename TElem,
                    typename TDim,
                    typename TIdx>
                    ...
                    mem::buf::BufCudaHipRt<TElem, TDim, TIdx>>
                {
                    ...
                    //-----------------------------------------------------------------------------
                    ALPAKA_FN_HOST static auto getPtrNative(
                        mem::buf::BufCudaHipRt<TElem, TDim, TIdx> & buf)
                    -> TElem *
                    {
                        return buf.m_spMem.get();
                    }
                ...
                //#############################################################################
                //! The BufCudaHipRt pointer on device get trait specialization.
                template<
                    typename TElem,
                    typename TDim,
                    typename TIdx>
                ...
                //#############################################################################
                //! The BufCudaHipRt pitch get trait specialization.
                template<
                    typename TElem,
                    typename TDim,
                    typename TIdx>
            ...
            namespace traits
            {
                    ...
                    dev::DevCudaHipRt>
                {
                    ...
                    ALPAKA_FN_HOST static auto alloc(
                        dev::DevCudaHipRt const & dev,
                        TExtent const & extent)
                    -> mem::buf::BufCudaHipRt<TElem, dim::DimInt<1u>, TIdx>
                    {
                        ALPAKA_DEBUG_MINIMAL_LOG_SCOPE;
                    ...
                    dev::DevCudaHipRt>
                {
                    ...
                    ALPAKA_FN_HOST static auto alloc(
                        dev::DevCudaHipRt const & dev,
                        TExtent const & extent)
                    -> mem::buf::BufCudaHipRt<TElem, dim::DimInt<2u>, TIdx>
                    {
                        ALPAKA_DEBUG_MINIMAL_LOG_SCOPE;
                    ...
                    dev::DevCudaHipRt>
                {
                    ...
                    ALPAKA_FN_HOST static auto alloc(
                        dev::DevCudaHipRt const & dev,
                        TExtent const & extent)
                    -> mem::buf::BufCudaHipRt<TElem, dim::DimInt<3u>, TIdx>
                    {
                        ALPAKA_DEBUG_MINIMAL_LOG_SCOPE;
                ...
                //#############################################################################
                //! The BufCudaHipRt CUDA device memory mapping trait specialization.
                template<
                    typename TElem,
                    typename TDim,
                    typename TIdx>
                ...
                //#############################################################################
                //! The BufCudaHipRt CUDA device memory unmapping trait specialization.
                template<
                    typename TElem,
                    typename TDim,
                    typename TIdx>
                ...
                //#############################################################################
                //! The BufCudaHipRt memory pinning trait specialization.
                template<
                    typename TElem,
                    typename TDim,
                    typename TIdx>
                ...
                //#############################################################################
                //! The BufCudaHipRt memory unpinning trait specialization.
                template<
                    typename TElem,
                    typename TDim,
                    typename TIdx>
                ...
                //#############################################################################
                //! The BufCudaHipRt memory pin state trait specialization.
                template<
                    typename TElem,
                    typename TDim,
                    typename TIdx>
                    ...
                    mem::buf::BufCudaHipRt<TElem, TDim, TIdx>>
                {
                    ...
                    ALPAKA_FN_HOST static auto isPinned(
                        mem::buf::BufCudaHipRt<TElem, TDim, TIdx> const &)
                    -> bool
                    {
                        ALPAKA_DEBUG_MINIMAL_LOG_SCOPE;
                ...
                //#############################################################################
                //! The BufCudaHipRt memory prepareForAsyncCopy trait specialization.
                template<
                    typename TElem,
                    typename TDim,
                    typename TIdx>
    ...
    namespace offset
    {
        ...
        {
            //#############################################################################
            //! The BufCudaHipRt offset get trait specialization.
            template<
                typename TIdxIntegralConst,
                typename TElem,
                typename TDim,
    ...
    namespace idx
    {
        ...
        {
            //#############################################################################
            //! The BufCudaHipRt idx type trait specialization.
            template<
                typename TElem,
                typename TDim,
                typename TIdx>
